...
import com.house.crawler.CrawlerService;
...
public class HouseServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("findByKeyword".equals(action)) {
			...
			List<HouseVO> list = houseSvc.findByKeyword(keyword, sortedCondition, whichPage);
			if (list.size() == 0) {
				req.setAttribute("warning", "查無資料，請重新查詢");
			} else {
				houseCount = list.get(0).getHouseCount();
			}
			req.setAttribute("houseCount", houseCount);
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("update".equals(action)) {
			...
			if (part_main_img.getSize() == 0) {
				HouseJNDIDAO dao = new HouseJNDIDAO();
		...
		if (action.equals("findByCondition_a")) {
			...
			if (req.getParameter("sortedCondition") != null) {
				...
				sortedCondition = "house_no";
			}

			if (req.getParameter("sortedCondition") == null) {

				HouseService houseSvc = new HouseService();
				List<HouseVO> list = houseSvc.findByCondition(location, sortedCondition, whichPage);
				houseCount = list.get(0).getHouseCount();
				if (list.size() == 0) {
					req.setAttribute("warning", "查無資料，請重新查詢");
				}
		...
		if (action.equals("findByCondition_b")) {
			...
			if (req.getParameter("sortedCondition") == null) {
				...
				List<HouseVO> list = houseSvc.findByPrice(price_str, price_end, sortedCondition, whichPage);
				if (list.size() == 0) {
					req.setAttribute("warning", "查無資料，請重新查詢");
				}
		...
		if (action.equals("findByCondition_c")) {
			...
			if (req.getParameter("sortedCondition") == null) {
				sortedCondition = "house_no";
				List<HouseVO> list = houseSvc.findByPattern(roomCount, sortedCondition, whichPage);
				if (list.size() == 0) {
					req.setAttribute("warning", "查無資料，請重新查詢");
				}
				houseCount = list.get(0).getHouseCount();
		...
		if (action.equals("findByCondition_d")) {
			...
			if (req.getParameter("sortedCondition") == null) {
				sortedCondition = "house_no";
				List<HouseVO> list = houseSvc.findByHouseType(houseType, sortedCondition, whichPage);
				if (list.size() == 0) {
					req.setAttribute("warning", "查無資料，請重新查詢");
				} else {
					houseCount = list.get(0).getHouseCount();
		...
		if (action.equals("findByCondition_e")) {
			...
			if (req.getParameter("sortedCondition") == null) {
				sortedCondition = "house_no";
				List<HouseVO> list = houseSvc.findByTotalPings(total_pings_str, total_pings_end, sortedCondition,whichPage);
				if (list.size() == 0) {
					req.setAttribute("warning", "查無資料，請重新查詢");
				} else {
					houseCount = list.get(0).getHouseCount();
		...
		if ("getAll".equals(action)) {
			...
			req.getAttribute("houseCount");

			HouseService houseSvc = new HouseService();
			String sortedCondition = "";
			Integer houseCount = houseSvc.getHouseCount();
			System.out.println("housecount = " + houseCount);
			if (req.getParameter("sortedCondition") == null) {
				sortedCondition = "house_no";
				List<HouseVO> list = houseSvc.getAll(sortedCondition, whichPage);
				req.setAttribute("houseCount", houseCount);
				req.setAttribute("whichPage", whichPage);
				req.setAttribute("list", list);
				req.setAttribute("action", action);
		...
		if ("findBySeveralConds".equals(action)) {
			...
			if (req.getParameter("sortedCondition") != null) {
				sortedCondition = req.getParameter("sortedCondition");
				list = houseSvc.findBySeveralConds(map, sortedCondition, whichPage);
				req.setAttribute("sortedCondition", sortedCondition);
			} else {
				sortedCondition = "house_no";
				list = houseSvc.findBySeveralConds(map, sortedCondition, whichPage);
		...
		if ("callSinyiCrawler".equals(action)) {
			CrawlerService crawSvc = new CrawlerService();
			crawSvc.getSinyiHouseInfo();
		}

		if ("callYunchingCrawler".equals(action)) {
			CrawlerService crawSvc = new CrawlerService();
			crawSvc.getYunchingHouseInfo();
		}
	}
	// doPost結束
}

	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("update".equals(action)) { // 來自update_promo_input.jsp的請求
			...
			try {
				 ...
				 * 1.接收請求參數 - 輸入格式的錯誤處理
				 *****************/
				Integer coupon_quanity = Integer.valueOf(req.getParameter("coupon_quanity").trim());
				String promo_no = req.getParameter("promo_no");
				...
				PromoService promoSvc = new PromoService();				
				if(promo_photo==null){
					PromoVO promoVO_org = promoSvc.getOne(promo_no);
					promo_photo= promoVO_org.getPromo_photo();					
				}				
				promoVO = promoSvc.update(promo_from, promo_to, promo_name, promo_content, promo_photo, promo_state,
						promo_no, emp_no,coupon_quanity);
